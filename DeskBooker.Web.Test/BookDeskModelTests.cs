...
namespace DeskBooker.Web.Test
{
    public class BookDeskModelTests
    {
        ...
        public void ShouldAddModelErrorIfNoDeskIsAvailable()
        {
            ...
            Assert.Equal("No desk available for the selected date", modelError.ErrorMessage);
        }
        ...
        [Fact]
        public void ShouldRedirectToBookConfirmationPage()
        {
            //Arrange
            _deskBookingResult.Code = DeskBookingResultCode.Success;
            _deskBookingResult.DeskBookingId = 7;
            _deskBookingResult.FirstName = "Nicolas";
            _deskBookingResult.Date = new DateTime(2021, 4, 4);
            ...
            IDictionary<string, object> routesValues = redirectToPageResult.RouteValues;
            Assert.Equal(3, routesValues.Count);

            var deskBookingId = Assert.Contains("DeskBookingId", routesValues);
            Assert.Equal(_deskBookingResult.DeskBookingId, deskBookingId);

            var firstName = Assert.Contains("FirstName", routesValues);
            Assert.Equal(_deskBookingResult.FirstName, firstName);

            var date = Assert.Contains("Date", routesValues);
